...
- [ ] 主 App target 有 `PrivacyInfo.xcprivacy`
- [ ] 每个 App Clip / Widget / Activity Extension / Share Extension 等独立 bundle 都有各自的 `PrivacyInfo.xcprivacy`（如适用）
...
**为什么重要**：2024 年 5 月起强制要求，缺失直接在 ASC 上传阶段被自动拦截。注意：Apple 按 bundle 检查，不是“项目里有一个 manifest 就算通过”。
