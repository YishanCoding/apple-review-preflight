...
# 栈识别速查：
#   .xcodeproj / .xcworkspace        → 原生 iOS（Info.plist 通常在 <Project>/Info.plist）
#   package.json + react-native      → React Native（ios/<AppName>/Info.plist）
#   package.json + expo              → Expo（ios/<AppName>/Info.plist 或 app.json）
#   capacitor.config.*               → Capacitor（ios/App/App/Info.plist）
#   pubspec.yaml                     → Flutter（ios/Runner/Info.plist）
...
# 3. 列出所有 PrivacyInfo.xcprivacy（每个 Extension/App Clip Target 都需独立 manifest，
#    主 App 有 ≠ 全部 Target 都有，是 ITMS-91053/91056 的高频拒审点）
find . -name "PrivacyInfo.xcprivacy" 2>/dev/null

# 4. 探测 Extension/Widget/App Clip Target（用于交叉验证 manifest 覆盖率）
find . -name "project.pbxproj" -not -path "*/Pods/*" -print0 2>/dev/null | \
  xargs -0 grep -l -E "WidgetExtension|LiveActivityExtension|AppClip|ShareExtension|NotificationServiceExtension" 2>/dev/null
# 详见 references/widgets-live-activity.md / references/app-clips.md

# 5. 检查 entitlements
find . -name "*.entitlements" -not -path "*/Pods/*" -not -path "*/node_modules/*" 2>/dev/null

# 6. 检查 package.json（RN/Expo/Capacitor 项目的高风险依赖）
cat package.json 2>/dev/null | python3 -c "
import json,sys
p=json.load(sys.stdin)
deps={**p.get('dependencies',{}),**p.get('devDependencies',{})}
keys=['purchase','subscription','auth','analytics','tracking','firebase','sentry','revenuecat','codepush','stripe','adjust','appsflyer']
for k,v in deps.items():
    if any(x in k.lower() for x in keys): print(f'{k}={v}')
" 2>/dev/null

# 7. 检查 Podfile / Package.swift（原生项目的高风险依赖）
test -f Podfile && grep -E "(Firebase|Facebook|Sentry|RevenueCat|Stripe|Adjust|AppsFlyer|CodePush)" Podfile
test -f Package.swift && grep -E "(Firebase|Facebook|Sentry|RevenueCat|Stripe|Adjust|AppsFlyer)" Package.swift

# 8. Flutter 项目（pubspec.yaml）
test -f pubspec.yaml && grep -E "(in_app_purchase|firebase|sign_in_with_apple|adjust|appsflyer)" pubspec.yaml
```
